...
void MainWindow::dateOgonki()
{
    ...
    for ( int i = 0; i < eModel->rowCount(); i++ ) {
        QModelIndex index = eModel->index(i, EmployeeModel::EColumn::Birthday);
        ...
        index = eModel->index(i, EmployeeModel::EColumn::Name);
        QString name = eModel->data(index, Qt::EditRole).toString();
        index = eModel->index(i, EmployeeModel::EColumn::Famili);
        QString famili = eModel->data(index, Qt::EditRole).toString();
        index = eModel->index(i, EmployeeModel::EColumn::Otch);
...
{
    QWidget *activeWidget = qApp->focusWidget(); // Получаем активный виджет
    if (activeWidget && activeWidget == ui->tableView) {
...
void MainWindow::init()
{
    ...
    connect(ui->tableView->model(), &QAbstractTableModel::dataChanged, [&]() {
        dateOgonki();
        if ( senderSignalIndex() == EmployeeModel::EColumn::Birthday )  {
//TODO при любом изменение вызывается dataOgonki (не прально!!!!!)
    ...
    ui->tableView->setItemDelegateForColumn(EmployeeModel::Gender, delegate);
}
...
//    QFile file("O:/06. Данные отделов/НИО ММНМ/Рабочие данные/Собственные программы/Murzin_AV/Base/Vacation.txt");
    ...
    for ( int i = 0; i < eModel->rowCount(); i++ ) {
        familiList.push_back(eModel->data(QModelIndex(eModel->index(i, EmployeeModel::Famili))).toString() + " " +
                             eModel->data(QModelIndex(eModel->index(i, EmployeeModel::Name))).toString() + " " +
                             eModel->data(QModelIndex(eModel->index(i, EmployeeModel::Otch))).toString()
                             );
    }
        ...
        for ( const QString &word : splitLine ) {
            if ( familiList.contains(word) ) {
                ...
                if ( firstDay.isValid() && firstDay.month() == month ) {
                    ...
                    employeeVacation->type = splitLine[it+4];

                    int haveDays = days.toInt();
                    for ( int i = 0; i <= firstDay.daysInMonth() -1; i++ ) {
                        if ( firstDay.day() > i )
                            employeeVacation->dayList.push_back(false);
                        else {
                            if ( haveDays >= 0 ) employeeVacation->dayList.push_back(true);
                            else                 employeeVacation->dayList.push_back(false);

                            haveDays--;
                        }
                    }
                    vModel->addItem(employeeVacation);
                }

                if ( (firstDay.isValid() && firstDay.month() != month) && (lastDay.isValid() && lastDay.month() == month) ) {
                    for ( int i = 0; i <= lastDay.daysInMonth() -1; i++ ) {
                        if ( lastDay.day() > i ) employeeVacation->dayList.push_back(true);
                        else                     employeeVacation->dayList.push_back(false);
                    }
                    vModel->addItem(employeeVacation);
                }
                ...
                if (word.startsWith("Мурзин") ) {
                    employeeVacation->color = *new QColor(255, 0, 204);
                }
                else if (word.startsWith("Яковлев" ) ) {
                    employeeVacation->color = *new QColor(61, 208, 154);
                }
                else if (word.startsWith("Малюгин" ) ) {
                    employeeVacation->color = *new QColor(0,0,255);
                }
                else if (word.startsWith("Жулин" ) ) {
                    employeeVacation->color = *new QColor(30,33,61);
                }
                else if (word.startsWith("Казначеев" ) ) {
                    employeeVacation->color = *new QColor(169,29,17);
                }
                else if (word.startsWith("Андреев" ) ) {
                    employeeVacation->color = *new QColor(169,131,7);
                }
                else if (word.startsWith("Малышева" ) ) {
                    employeeVacation->color = *new QColor(147,170,0);
                }
                else if (word.startsWith("Белоусова" ) ) {
                    employeeVacation->color = *new QColor(76,187,23);
                }
                else if (word.startsWith("Кондратьев" ) ) {
                    employeeVacation->color = *new QColor(0,255,255);
                }
                else if (word.startsWith("Рыбачук" ) ) {
                    employeeVacation->color = *new QColor(150,186,181);
                }
                else if (word.startsWith("Коняхина" ) ) {
                    employeeVacation->color = *new QColor(102,0,153);
                }
                else if (word.startsWith("Резанов" ) ) {
                    employeeVacation->color = *new QColor(19, 10, 143);
                }
                else if (word.startsWith("Карпов" ) ) {
                    employeeVacation->color = *new QColor(19, 136, 8);
                }
            }
            it++;
        }
    ...
    VacationDelegate *vDelegate = new VacationDelegate(vModel->getEmployeeVacationList(), this);
...
void MainWindow::on_pb_Sort_clicked()
{
    if ( sortClickCount % 2 == 0) {
        eModel->sort(EmployeeModel::Birthday, Qt::AscendingOrder, true);
        ui->pb_Sort->setIcon(QIcon(":/src/resources/Icon/sort-ascending.png"));
    }
    else {
        eModel->sort(EmployeeModel::Birthday, Qt::DescendingOrder, true);
...
void MainWindow::on_pb_Send_clicked()
{
    ...
    for ( const QModelIndex& index : selectedIndexes ) {
        if ( index.column() == EmployeeModel::Email ) {
    ...
    QString name = eModel->data(index.sibling(index.row(), EmployeeModel::Name), Qt::DisplayRole).toString();
    QString famili = eModel->data(index.sibling(index.row(), EmployeeModel::Famili), Qt::DisplayRole).toString();
    QString otch = eModel->data(index.sibling(index.row(), EmployeeModel::Otch), Qt::DisplayRole).toString();
    QString birthday = eModel->data(index.sibling(index.row(), EmployeeModel::Birthday), Qt::DisplayRole).toString();
